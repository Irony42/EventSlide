...
| 2   | `web/src/lib/api/client.ts`                                                           | Typed `uploadPhotos(slug, files, caption)`; builds `FormData`. One method per endpoint, and the only place this endpoint's shape is written in the web app.                                                                                                                                                                                                                                                                                                   |
| 3   | `web/src/lib/http.ts`                                                                 | `credentials: 'same-origin'`, CSRF header, and an `ApiError` carrying the server's `error.code` and nothing else — the sentence is chosen at render time by `messageForCode(code, table)`, in whichever language the reader is in. The only place `fetch` appears.                                                                                                                                                                                            |
...
| 5   | `src/interface/http/server.ts`                                                        | Middleware order is load-bearing: `securityHeaders` → `permissionsPolicy` → `requestContext` → **health** → `express.json` → `cookieParser` → `session` → `issueCsrfToken` → `attachUser` → `requireCsrfToken` → routers → `apiNotFound` → SPA fallback → `errorHandler`. Health is mounted **before** CSRF because a readiness probe carries no token; `apiNotFound` precedes the SPA fallback, or `/api/nope` would answer `200` with `index.html`.         |
| 6   | `requestContext` in `src/interface/http/middleware/errorHandler.ts`                   | Sets the per-request id and the bounded client id (`/^[A-Za-z0-9_-]{1,64}$/`). Every log line and every 5xx carries the request id; the response never carries the underlying error. Mounted ahead of the body parser, so a request the parser rejects still has an id to log.                                                                                                                                                                                |
...
| 8   | `src/interface/http/routes/guestRoutes.ts`                                            | `router.post('/events/:eventSlug/photos', uploadLimiter(...), requireGuest(deps), uploads.array(PHOTOS_FIELD), asyncHandler(handler))`. `asyncHandler` is mandatory: a bare `async` handler turns a rejection into an unhandled rejection and kills the process.                                                                                                                                                                                              |
| 9   | `requireGuest` in `src/interface/http/middleware/authz.ts`                            | Reads the `HttpOnly; SameSite=Lax` cookie (`GUEST_COOKIE`), verifies the HMAC through the `GuestTokenService` port, and populates the request context. Bad signature → `401`. Token verification and event scoping are one middleware, not two.                                                                                                                                                                                                               |
...
| 11  | `multer` configured in `src/interface/http/routes/guestRoutes.ts`                     | `multer.memoryStorage()` with `limits: { fileSize, files }`. **In memory, not on disk**: bytes are re-encoded and content-addressed before anything is written, so a temp file would be a second place for a half-written photo to exist. **No `fileFilter` on MIME type** — the client's `Content-Type` is not evidence and is never consulted.                                                                                                              |
| 12  | `src/interface/http/schemas/requestSchemas.ts`                                        | `.parse(req.params)` (slug regex) and `.parse(req.body)` (caption bound). Shape only; "the event must accept uploads" is a domain rule, never a `z.refine`. Every schema is `.strict()`, so an unexpected field is a `400` rather than a silently ignored one.                                                                                                                                                                                                |
...
| 16  | `event.quotaBytes` (`src/domain/events/event.ts`) + `photos.countBytes(eventId)`      | Remaining bytes vs. the sum of this event's `byte_size`. Over → `event.quotaExceeded` → `413`. The event stops accepting uploads instead of filling the disk. The quota lives on the entity rather than in a module of its own — it is a property of the event, not a free function.                                                                                                                                                                          |
...
| 24  | `application/ports/eventBus.ts` → `infrastructure/realtime/inMemoryEventBus.ts`       | `bus.publish({ type: 'photo.submitted', eventId, photoId })`. Side effects come **after** persistence: a subscriber must never learn about an uncommitted row.                                                                                                                                                                                                                                                                                                |
| 25  | `interface/http/routes/streamRoutes.ts`                                               | Subscribed to the bus at container wiring. Maps the domain event to a frame on this event's **`moderation`** topic — not `wall`, because the wall must not learn that a pending photo exists. There is no separate hub module: the per-event channels, the heartbeat and `Last-Event-ID` replay all live with the route that serves them.                                                                                                                     |
| 26  | `interface/http/presenters/presenters.ts`                                             | `201 { results: UploadOutcomeDto[] }` — one outcome per file, because a guest sending eight photos on venue Wi-Fi partially succeeding is the normal case, not an error. Absent from every DTO: storage key, absolute path, uploader IP, raw EXIF.                                                                                                                                                                                                            |
...
| 31  | `application/usecases/moderation/moderatePhoto.ts`                                    | The host clicks _Publier_: the decision goes through the status machine, then `bus.publish({ type: 'photo.published' })` → the **`wall`** topic. One use case for every decision, so the transition table is consulted in exactly one place.                                                                                                                                                                                                                  |
| 32  | `web/src/features/wall/hooks/useWallPlaylist.ts`                                      | Invalidation → refetch `?status=published` → `src/domain/slideshow/` (pure) recomputes the playlist → the photo is on the projector.                                                                                                                                                                                                                                                                                                                          |
...
strings; the wording is chosen in `web/src/lib/i18n/`, per language, which is also why
...
| Entity | `User` + `Membership`        | `users/`                                        | A role inside an event is always _per event_; `User.siteRole` is authority over the **box**, never over an event |
...
`src/interface/http/routes/streamRoutes.ts` over `src/infrastructure/realtime/inMemoryEventBus.ts`.
...
const schema = z.object({
  ...
  CLIP_MAX_HEIGHT: …, // 720
  // The counterpart of MAX_IMAGE_PIXELS, and not the same control as CLIP_MAX_HEIGHT:
  // that one scales the output, and `-vf scale` runs after the decoder has allocated
  // the frame. Judged from the probed header, before a frame exists.
  MAX_CLIP_PIXELS: …, // 33_177_600 — 8K UHD
...
- **`TRUST_PROXY_HOPS` is a count, not a boolean**, because `express-rate-limit` keys on
  the client IP. Trusting `X-Forwarded-For` when nothing sets it lets a client forge its
  own rate-limit bucket; not trusting it behind a reverse proxy rate-limits the whole
  venue as one IP. A hop count says how many entries at the end of that header the
  deployment actually put there, which `true` does not — so the default is `0`, and an
  operator behind one proxy sets `1`.
