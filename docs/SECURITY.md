...
| A leaked display URL exposes published photos                      | the wall is meant to be looked at; a projector cannot log in                                           | display access can require the join code for private events; only `published` photos are ever served                                                                          |
