...
| 3   | The host's own switch: `settings.allowClips`                                                                                                                                                | `usecases/clips/uploadClip.ts`                   | 403 `event.clipsNotAllowed`                                                        |
...
| 5   | Backpressure: the process-wide queue depth                                                                                                                                                  | `domain/clips/clipQueue.ts`                      | **429 `clip.queueFull` with `Retry-After`** — never the quota's 413                |
| 6   | The byte quota, counting the staged sources already in the queue as well as the album                                                                                                       | `uploadClip.ts`, `sqlitePhotoRepository.ts`      | 413 `event.quotaExceeded`                                                          |
| 7   | Source written to the media store as the unservable `source` variant, **then** the `clip_jobs` row. The multer temp file is unlinked before the response, and again in a `finally`          | `uploadClip.ts`, `clipRoutes.ts`                 | 500 `clip.stageFailed`; the staged bytes are removed                               |
| 8   | `202 Accepted`. **No `photos` row exists**, so nothing can reach a moderator or the wall                                                                                                    | `clipRoutes.ts`                                  | —                                                                                  |
| 9   | The worker probes with `ffprobe`, under a pinned demuxer and `-protocol_whitelist file`, and applies the duration cap to the header                                                         | `ffmpegVideoTranscoder.ts`                       | `clip.corrupt`, `clip.noVideoStream`, `clip.tooLong`, `clip.tooShort`              |
| 10  | Re-encode to H.264/AAC in an 8-bit 4:2:0 mp4, dropping every stream that is not the picture or the sound, bounded by `-t` and `-fs`, under a wall-clock and a stall timeout                 | same                                             | `clip.transcodeFailed`, `clip.transcodeTimedOut`                                   |
| 11  | Poster frame cut from the **output**, then both files hashed and stored, then the `photos` row through `saveManyWithinLimits` — quota counted inside the insert transaction, as for a photo | `usecases/clips/transcodeNextClip.ts`            | `clip.storageFailed`; `event.quotaExceeded`; everything this pass wrote is removed |
...
- **The guest's upload is never servable.** It is stored as the `source` variant, which is
  ...
  wrote into the container. It is deleted the moment the transcode succeeds, or when the
  clip is given up on.
...
**The clip quota is enforced exactly once, on the transcoded output, and the staged
source is charged on a check that two requests can both pass.** The upload path reads
`photos.totalBytes` — which spans the queue as well as the album — and then writes; two
uploads in flight can therefore both see room that only one of them has. What bounds the
overshoot is the queue depth: at most `MAX_QUEUED_CLIPS` sources can be staged at all, so
the worst case is `MAX_QUEUED_CLIPS x MAX_CLIP_BYTES` beyond the quota (1.6 GB at the
defaults) before the queue refuses everything. The transcoded output is decided inside the
insert transaction, as a photo is, so nothing beyond that window is unbounded. A per-event
transactional reservation at staging would close it and is not built.
...
| A clip still waiting for the transcoder             | it is the guest upload, on its way      | minutes — deleted the moment the transcode succeeds or is given up on, and **never servable** (§4.1) |
...
| A burst of clip uploads can overshoot an event byte quota           | the staged source is charged on a check two requests in flight can both pass, and the enforcing transactional check is on the transcoded output. Closing it needs a per-event reservation written inside the staging transaction, which is a second quota mechanism beside the one that already holds for photographs | the overshoot is bounded by the queue depth — at most `MAX_QUEUED_CLIPS x MAX_CLIP_BYTES` beyond the quota before the queue refuses everything — and the staged bytes are released as each clip finishes or fails (§4.1) |
