...
| A disabled account holds no authority anywhere, on the next request                                                                              | 2, 3, 4, 6    | `membershipRepositoryContract` and `userRepositoryContract` run the rule against the fake **and** SQLite, which is the only thing keeping the two honest about a join the fake has to simulate; `registerModerator.test.ts` holds the escalation (a disabled owner minting an enabled account); `authz.test.ts` holds the 404 and the 401, and that the disabled member's refusal is byte for byte the stranger's; `tenant-isolation.spec.ts` switches the account off in the running server's own database                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| A host session ends at an absolute age, however busy it has been                                                                                 | 4             | `authz.test.ts` drives `FakeClock` past the cap, and asserts the cap is **not** pushed forward by use — which is the whole difference from the idle timeout that made this window unbounded                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| An unconfigured boot is refused, never quietly relaxed                                                                                           | 3             | `env.test.ts` — an environment naming no `NODE_ENV` is production, and its refusal names both secrets; two development boots never share a secret, so no constant can come back                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| The box's operator holds nothing inside an event they are not a member of                                                                        | 4, 6          | `siteOperatorScope.test.ts` sweeps **every** route off the assembled server, once in each `SITE_ADMIN` mode — not the ones spelling `:eventSlug` — and asserts no refusal came from the CSRF gate ahead of it, which is what made the mutating half of the sweep vacuous; `authz.test.ts` adds a `CallLog` assertion that `requireRole` never even asks for a site role; `tenant-isolation.spec.ts` carries the end-to-end half, reading the operator's role out of the running server's database so the premise is checked rather than assumed                                                                                                                                                                                                                                                                                                                                                                                                          |
| Site administration is operator-only when `SITE_ADMIN=on`, an unknown route when off, and no other router may claim `/api/site` or `/api/site/*` | 4 (6 pending) | `siteAdminMode.test.ts` drives both modes on paths no route claims, with every method: off, byte for byte what an unknown route answers; on, `401` / `403` from the gate itself, and `500` rather than a fall-through when the role cannot be read. `container.test.ts` holds that the parsed switch reaches `buildServer`, and that both modes apply every migration. A structural case in `siteOperatorScope.test.ts` walks the assembled server in both modes and fails on any route at `/api/site` or `/api/site/*` that another router declares — the `/api//site/x` request that mount order cannot catch. Its walk refuses to guess: a route path that is anything but literal segments and `:parameters`, a middleware mounted at a path inside a router, or a sub-app makes it fail loudly rather than pass on a shape it cannot place. Its `OPERATOR_ROUTES` list is checked per mode. Ring 6 is pending: the e2e mode matrix of roadmap §10.9 |
