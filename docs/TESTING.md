...
| The box's operator holds nothing inside an event they are not a member of       | 4, 6       | `siteOperatorScope.test.ts` sweeps every event-scoped route off the assembled server; `authz.test.ts` adds a `CallLog` assertion that `requireRole` never even asks for a site role; `tenant-isolation.spec.ts` carries the end-to-end half |
