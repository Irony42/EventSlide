...
| 15  | A screenshot does not freeze an animation — `animations: 'disabled'` calls `finish()` and photographs the **end** frame. A visual baseline over an animated surface pins the input that drives the animation and asserts the surface is at rest before the shutter. `--wall-transition: 0` does not cover the filmstrip's drift, which is timed from the slide interval (`e2e_interval=0` is what stops it). Measured: one shot moved the band 384px — one whole frame, 34% of the screen — from identical DOM. |
