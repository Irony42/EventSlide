...
export interface VideoTranscoder {
  /**
   * Whether this deployment can transcode at all: is there an encoder on this box.
   *
   * Decided once at boot by the capability check and answered from a boolean here, so it
   * is as cheap as {@link VideoTranscoder.identify} and can be asked on the request path.
   *
   * It exists so an upload can be **refused while the guest is still holding the
   * request**. Without it a box with no ffmpeg answers `202` to every clip, queues it,
   * and the guest learns twenty seconds later by polling that a job they were told was
   * accepted has failed — after pushing eighty megabytes over venue Wi-Fi. The honest
   * answer is `500 clip.transcoderUnavailable` before any of that.
   *
   * Deliberately **not** something readiness acts on: a photo wall with no video still
   * serves the room, and taking a venue out of service over a missing codec is a worse
   * outage than the one it reports.
   */
  available(): boolean
