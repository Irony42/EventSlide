...
export interface MediaMetadata {
  ...
  /**
   * When these bytes were last written — the same fact {@link StoredObject} carries.
   *
   * It is here so the reconciliation sweep can **re-ask** immediately before it unlinks:
   * the listing that decided an object was collectable is a snapshot, and a guest whose
   * clip was abandoned may re-upload it in the meantime, rewriting the same
   * content-addressed path with a fresh timestamp. Deleting against the stale answer
   * leaves the new job pointing at nothing, and `clip.sourceMissing` is permanent.
   */
  readonly modifiedAt: Date
}

/**
 * One object the store is holding, as the reconciliation sweep sees it.
 *
 * `modifiedAt` is the load-bearing field and the reason this is not just a list of
 * hashes. Every write path in the product is **bytes first, row second** — the ingest
 * pipeline, the clip upload, the transcode — so there is always an instant in which an
 * object is on the disk and nothing in the database names it yet. A collector with no
 * notion of age would eat the file out from under the insert that was about to reference
 * it, which is a far worse bug than the leak it was written to fix.
 */
export interface StoredObject {
  readonly hash: ContentHash
  readonly variant: MediaVariant
  readonly byteSize: number
  /** When these bytes were last written. A fresh object is never collected. */
  readonly modifiedAt: Date
}
...
export interface MediaStore {
  ...
  /**
   * Every event this store is holding bytes for.
   *
   * The enumeration side of reconciliation, and deliberately asked of the **store**
   * rather than of the events table: the question is "what is on the disk", and an
   * answer derived from the database could never name an object the database has
   * forgotten — which is the entire class of thing the sweep exists to find.
   */
  listEvents(): Promise<readonly EventId[]>

  /**
   * Every object stored for one event.
   *
   * Feeds `sweepOrphanedMedia`, which is the collector three separate paths in this
   * codebase now depend on rather than deleting files themselves. Objects the store
   * cannot name — a leaked `.tmp` from an interrupted `put`, a file an operator dropped
   * in by hand — are **not** listed: this is the inventory of things the store owns, and
   * a sweep must not be handed a filename it could not have written.
   */
  list(eventId: EventId): Promise<readonly StoredObject[]>
}
