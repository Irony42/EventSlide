...
export const videoTranscoderContract = (
  ...
  describe(`VideoTranscoder contract: ${name}`, () => {
    ...
    describe('available', () => {
      it('reports that a working transcoder can transcode', () => {
        // A subject handed to this suite is one with an encoder behind it, so the
        // interesting answer is the negative one — and that belongs to the Null Object,
        // which has its own test. What this holds is that the question is answerable at
        // all and costs nothing: the upload route asks it on the guest's request.
        expect(subject.transcoder.available()).toBe(true)
      })
    })
