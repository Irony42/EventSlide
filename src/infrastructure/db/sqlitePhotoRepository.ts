 ...
 *
 * For the transcode worker, and only for it: it inserts a clip's output while that
 * clip's own job is still `running`, so the plain sum charges the event for the source
 * *and* the result it became. A source is routinely twenty times its 720p output, so an
 * event anywhere near its quota refused a clip that plainly fitted.
 *
 * `id IS NOT :excludeClipJobId` rather than `id <> :excludeClipJobId`, because `IS NOT`
 * is the null-safe comparison: with the parameter bound to NULL it reads
...
export class SqlitePhotoRepository implements PhotoRepository {
  ...
  async findIdsReferencing(eventId: EventId, hash: ContentHash): Promise<readonly PhotoId[]> {
    // `poster_hash` is in the predicate and not only `content_hash`: a poster is the one
    // digest this table can hold twice. Event-scoped like everything else here — two
    // events each own their copy of identical bytes, and one must never decide the
    // other's file may go.
    // **A union of two seeks, not one `OR`.** SQLite will not use two different indexes
    // for an `OR` across two columns, so the single-predicate form scanned the event's
    // whole album on every photo delete and on every object the reconciliation sweep
    // considers. `UNION` lets each arm take its own index — `idx_photos_event_hash` and
    // `idx_photos_event_poster` — and dedupes the row a clip matches on both.
    return this.db
      .prepare<[string, string, string, string], { readonly id: string }>(
        `SELECT id FROM photos WHERE event_id = ? AND content_hash = ?
          UNION
         SELECT id FROM photos WHERE event_id = ? AND poster_hash = ?`,
      )
      .all(eventId, hash.value, eventId, hash.value)
      .map((row) => asPhotoId(row.id))
  }

  async listReferencedDigests(eventId: EventId): Promise<ReadonlySet<string>> {
    // Both columns in one pass. `poster_hash` is NULL for every photograph, and
    // filtering it out here keeps the set the size of the digests that exist rather than
    // one entry larger than the album.
    const rows = this.db
      .prepare<[string], { readonly content_hash: string; readonly poster_hash: string | null }>(
        `SELECT content_hash, poster_hash FROM photos WHERE event_id = ?`,
      )
      .all(eventId)

    const digests = new Set<string>()
    for (const row of rows) {
      digests.add(row.content_hash)
      if (row.poster_hash !== null) digests.add(row.poster_hash)
    }
    return digests
  }
  ...
  async saveManyWithinLimits(
    ...
    const insert = this.db.prepare<PhotoBindings>(INSERT_PHOTO)
    const sumBytes = this.db.prepare<
      { readonly eventId: string; readonly excludeClipJobId: string | null },
      AggregateRow
    >(SUM_EVENT_BYTES_CREDITING_CLIP)
