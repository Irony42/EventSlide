...
export class SqliteClipJobRepository implements ClipJobRepository {
  ...
  async findBySourceHash(eventId: EventId, sourceHash: ContentHash): Promise<ClipJob | null> {
    // The status filter is the port's contract and it is also what makes the partial
    // unique index usable: SQLite takes a partial index only when the query's WHERE
    // implies the index's, so spelling the same three statuses here turns this from a
    // scan of every clip the venue ever accepted into a seek.
    const row = this.db
      .prepare<[string, string], ClipJobRow>(
        `SELECT ${CLIP_JOB_COLUMNS}
           FROM clip_jobs
          WHERE event_id = ? AND source_hash = ?
            AND status IN (${BLOCKING_REUPLOAD_SQL})`,
      )
  ...
  async deleteForPhoto(eventId: EventId, photoId: PhotoId): Promise<void> {
    // Scoped by event as well as by photo, like every other write here: deleting is not
    // a licence to reach into another event's row.
    this.db
      .prepare<[string, string]>(`DELETE FROM clip_jobs WHERE event_id = ? AND photo_id = ?`)
      .run(eventId, photoId)
  }

  /**
   * One `UPDATE`, and `changes` is the answer.
   *
   * Update-**only**: see the port. There is no insert here to race with, so there is no
   * transaction either — a single statement is already atomic.
   */
  async save(job: ClipJob): Promise<boolean> {
    return this.db.prepare<ClipJobBindings>(UPDATE_CLIP_JOB).run(toBindings(job)).changes > 0
  }

  /**
   * The reservations a dead request left behind, deleted and returned in one transaction.
   *
   * `RETURNING` would be tidier and is deliberately not used: it needs SQLite 3.35, and
   * the bundled `better-sqlite3` is newer than that but the version an operator's own
   * build links against is not something this file gets to assume. Selecting inside the
   * transaction is the same guarantee with none of that.
   */
  async deleteStaleReservations(olderThan: Date): Promise<readonly ClipJob[]> {
    const stale = this.db.prepare<[string], ClipJobRow>(
      `SELECT ${CLIP_JOB_COLUMNS}
         FROM clip_jobs
        WHERE status = 'reserved' AND created_at <= ?
        ORDER BY created_at ASC`,
    )
    const remove = this.db.prepare<[string]>(`DELETE FROM clip_jobs WHERE id = ?`)

    return this.db
      .transaction((): readonly ClipJob[] => {
        const rows = stale.all(toIsoText(olderThan))
        for (const row of rows) remove.run(row.id)
        return rows.map(toClipJob)
      })
      .immediate()
  }
  ...
  async stagedBytesOf(eventId: EventId, clipJobId: ClipJobId): Promise<number> {
    const row = this.db
      .prepare<[string, string], CountRow>(
        `SELECT source_byte_size AS value
           FROM clip_jobs
          WHERE event_id = ? AND id = ? AND status IN (${HOLDING_BYTES_SQL})`,
      )
      .get(eventId, clipJobId)
  ...
  async listStagedSources(eventId: EventId): Promise<ReadonlySet<string>> {
    const rows = this.db
      .prepare<[string], { readonly source_hash: string }>(
        `SELECT source_hash
           FROM clip_jobs
          WHERE event_id = ? AND status IN (${HOLDING_BYTES_SQL})`,
      )
      .all(eventId)
