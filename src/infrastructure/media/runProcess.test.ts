...
describe('runProcess', () => {
  ...
  it('keeps draining a stream far larger than a pipe buffer', async () => {
    ...
    expect(result.ok).toBe(true)
    // Drained to the end **and kept whole**. stdout is a document a caller parses, not a
    // diagnostic: an 8 KB tail of it handed `JSON.parse` a file with its head cut off,
    // and an ordinary iPhone clip's `ffprobe` answer is 7.9 KB.
    expect(result.stdout.length).toBe(2 * 1024 * 1024)
    expect(result.stdoutTruncated).toBe(false)
  })

  it('bounds stdout rather than growing to whatever the child writes', async () => {
    // Drained is not unbounded. The budget is large because the answer is large, not
    // because there is no budget: a child that writes forever must not be able to grow
    // this process's heap until the container kills it.
    const result = await runProcess({
      binary: NODE,
    ...
    // And it says so, which is what lets a parser refuse instead of guessing.
    expect(result.stdoutTruncated).toBe(true)
  })

  it('reports truncation even when the overflow arrived as a single chunk', async () => {
    // The drop loop stops at one chunk, so a lone chunk past the budget was kept whole
    // here and head-cut later by the tail — leaving `stdoutTruncated` false about output
    // that had in fact lost its beginning. That flag is the only thing that separates
    // "this is what ffprobe said" from "this is the end of what ffprobe said", and the
    // second must be a transient `clip.probeUnreadable` rather than a permanent
    // `clip.corrupt` verdict on a guest's clip.
    const result = await runProcess({
      binary: NODE,
  ...
  it('keeps the end of a stream, which is where a failure says why', async () => {
    const result = await node(
      'process.stderr.write("x".repeat(200000)); process.stderr.write("THE REASON")',
      GENEROUS,
    )

    expect(result.stderr.endsWith('THE REASON')).toBe(true)
    // stderr keeps its small tail: it is a reason to log, never a document to parse.
