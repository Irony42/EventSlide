...
describe('nullVideoTranscoder', () => {
  ...
  it('says it cannot transcode, which is what the upload route asks before it writes', async () => {
    // The negative answer to `available()`, and the only implementation that gives one.
    // Without it a box with no ffmpeg answers 202 to every clip and the guest learns by
    // polling, twenty seconds and eighty megabytes later, that it had already failed.
    expect(transcoder.available()).toBe(false)
  })
