...
class Tail {
  ...
  append(chunk: Buffer): void {
    ...
    // **The flag is set from the size, not from the dropping.** The loop below stops at
    // one chunk — it has nothing left to drop — so a single chunk larger than the budget
    // stayed whole here and was head-cut by `toString`'s `subarray(-limit)` instead, with
    // `truncated` still false. Unreachable at a 4 MB budget against a 64 KB pipe read,
    // but this flag is what makes `clip.probeUnreadable` reachable rather than a clip
    // being called corrupt, so it must not have a branch that under-reports.
