...
/** A reservation: the row as `create` makes it, before the bytes have been written. */
const reserved = (overrides: Partial<NewClipJob> = {}): ClipJob =>
  must(ClipJob.create(newJob(overrides), asClipJobId('job-1'), asPhotoId('photo-1'), AT))

/**
 * A job whose bytes have landed — what the upload path leaves behind, and where every
 * rule below starts. `create` alone is a reservation the worker cannot see.
 */
const aJob = (overrides: Partial<NewClipJob> = {}): ClipJob => reserved(overrides).ready(AT)
...
describe('ClipJob.create', () => {
  it('reserves a clip: charged to the quota, and invisible to the worker', () => {
    // Reserve-then-write. The row exists before the bytes so the quota — which is
    // computed from rows — counts the upload from the instant it is admitted; and the
    // worker must not be able to claim a job whose source may not be there yet.
    const job = reserved()
    expect(job.status).toBe('reserved')
    expect(job.attempts).toBe(0)
    expect(job.isDue(AT)).toBe(false)
  })

  it('becomes claimable once its bytes have landed', () => {
    const job = reserved().ready(AT)
    expect(job.status).toBe('queued')
    expect(job.isDue(AT)).toBe(true)
  })

  it('throws rather than answering when it is readied twice', () => {
    // A refusal here is a programming error, not an outcome anybody can cause: there is
    // one caller and it runs once, straight after `create`. Throwing is what stops it
    // being mistaken for something the upload path should report to a guest.
    const queued = reserved().ready(AT)
    expect(() => queued.ready(AT)).toThrow(/not a reservation/)
  })
  ...
  it('fixes the photo id at staging, before any transcode has run', () => {
    ...
    expect(reserved().photoId).toBe('photo-1')
  })

  it('keeps the guest caption, so it survives the queue', () => {
    expect(reserved().caption?.value).toBe('Le premier slow')
  })

  it('records no failure and no reviewer decision yet', () => {
    expect(reserved().failureCode).toBeNull()
  })
