...
export class ClipJob {
  ...
  /**
   * A **reservation**: the row that is written before the bytes are.
   *
   * Not claimable, and deliberately so — the source may not be on the disk yet. It is
   * charged to the event's quota from this instant, which is the whole point: the quota
   * is computed from rows, so a row that appeared only after the bytes meant every
   * refused upload had already written megabytes nothing counted. The state machine in
   * `clipJobStatus.ts` has the whole argument.
   */
  static create(
    input: NewClipJob,
    id: ClipJobId,
  ...
  /**
   * The bytes are on the disk. Now the worker may have it.
   *
   * The second half of reserve-then-write, and the only move a reservation can make. A
   * job that never reaches here has a row nothing can claim and bytes that may not
   * exist; the reservation reaper deletes the row on a short timer, and the
   * reconciliation sweep collects anything that did land under its digest.
   */
  ready(now: Date): ClipJob {
    const queued = this.moveTo('queued', now, {})
    // **Throws rather than answering a `Result`**, unlike every other transition here.
    // The others are outcomes a guest or a host can cause; this one has a single caller,
    // immediately after `create`, so a refusal is a programming error and nothing the
    // upload path could sensibly report. The same choice `FakeClipJobRepository.claimNext`
    // makes about a row it has just proved is claimable.
    if (!queued.ok) throw new Error(`clip job ${this.props.id} is not a reservation`)
    return queued.value
  }
