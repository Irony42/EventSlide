...
const buildSubject = (options: SubjectOptions = {}): Subject => {
  ...
  // Both directions: the album counts the queue, and staging is judged against the
  // album. One statement in the SQLite adapter; two wired fakes here.
  clips.chargePhotoBytesFrom(photos)
  const media = new InMemoryMediaStore()
  const ids = new SequentialIdGenerator()
  const transcoder = new FakeVideoTranscoder()
  if (options.transcoderUnavailable === true) transcoder.unavailable()
...
describe('POST /api/events/:eventSlug/clips', () => {
  ...
  describe('authorization', () => {
    ...
    it('answers 500 when this deployment has no video encoder, and keeps nothing', async () => {
      // `docs/API.md` documents this status on this route, and until the capability was
      // asked on the request path it could not happen: the box answered 202, queued the
      // clip, and the guest found out by polling. The temp file still goes, because the
      // `finally` owns it on every exit path.
      const subject = buildSubject({ transcoderUnavailable: true })
      ...
      expect(response.body.error.code).toBe('clip.transcoderUnavailable')
      expect(subject.clips.all).toEqual([])
      expect(await readdir(subject.tempDir)).toEqual([])
    })
...
describe('GET /api/events/:eventSlug/clips/:clipJobId', () => {
  ...
  it('lets a guest poll a job staged by another guest at the same event', async () => {
    // Authorized by event membership, not authorship. The dedupe is event-scoped, so the
    // second guest to send the same video from the group chat is handed the first's job
    // id in a `202` — and an authorship check then answered their next poll with
    // "cette vidéo n'existe plus", about an upload the server had just accepted.
    const subject = buildSubject()
    seedJobs(subject)
    ...
    expect(response.status).toBe(200)
    expect(response.body.clipJobId).toBe(OTHERS)
  })
